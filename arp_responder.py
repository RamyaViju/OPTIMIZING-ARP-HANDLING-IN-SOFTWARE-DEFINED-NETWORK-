        ...
        if a.protosrc != 0:
          ...
          if a.opcode == arp.REQUEST:
            ...
            if a.protodst in _arp_table:
              ...
              # In our case if entry is absent in ARP table then
              # there is no need to populate failed queries.
...
def launch (timeout=ARP_TIMEOUT, no_flow=False, eat_packets=True,
  ...
  for k,v in kw.iteritems():
    # because we need to make ARP cache dynamic and
    # not static
    # _arp_table[IPAddr(k)] = Entry(v, static=True)
    _arp_table[IPAddr(k)] = Entry(v, static=False)
